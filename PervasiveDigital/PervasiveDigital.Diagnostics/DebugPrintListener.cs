    ...
    public class DebugPrintListener : ITraceListener
    {
        ...
        public void RecordEvent(byte[] buffer)
        {
            ...
            if (_eventDescriptors.Contains(evt.EventTag))
            {
                ...
                Debug.WriteLine(StringUtilities.Format("{0} {1} : {2}", evt.Timestamp.ToString("G"), SevToString(evt.Severity), str));
            }
            else
            {
                Debug.WriteLine(StringUtilities.Format("{0} {1} : {2}", evt.Timestamp.ToString("G"), SevToString(evt.Severity), evt.EventTag));
                for (var i = 0 ; i<evt.Args.Length ; ++i)
                {
                    Debug.WriteLine(StringUtilities.Format("    [{0}] : {1}", i, evt.Args[i].ToString()));
                }
            }
        }
