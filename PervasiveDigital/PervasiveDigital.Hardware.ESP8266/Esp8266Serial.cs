...
namespace PervasiveDigital.Hardware.ESP8266
{
    ...
    internal class Esp8266Serial
    {
        ...
        private readonly UartController _port;
        ...
        public Esp8266Serial(UartController port)
        {
...
//#endif
        ...
        private void _port_DataReceived(UartController sender, DataReceivedEventArgs e)
        {
            byte[] newInput = new byte[sender.BytesToRead];
            int bytesReceived = 0;
            if (sender.BytesToRead > 0)
            {            
                bytesReceived = sender.Read(newInput, 0, sender.BytesToRead);
               
            }

            if (bytesReceived > 0) _buffer.Put(newInput);
